...
module.exports.deferPurchase = function (dPubkey, receipt, extendTime, cb) {

	verbose.log(NAME, 'Extend this:', receipt, 'by: ', extendTime);
	...
	auth(tokenMap, function (error, res) {
		...
		const expiryUrl = 'https://www.googleapis.com/androidpublisher/v2/applications/' +
			encodeURIComponent(receipt.data.packageName) +
			'/purchases/subscriptions/' +
			encodeURIComponent(receipt.data.productId) +
			'/tokens/' + encodeURIComponent(receipt.data.purchaseToken) +
		...
		request.get({
			...
			if (error || !body.expiryTimeMillis) {
				verbose.log(NAME, 'Failed: validating initial receipt');
				return cb(new Error('validating initial receipt'), {
					status: constants.VALIDATION.FAILURE,
					message: 'Malformed receipt'
				});
			}
			const expiryTime = Number(body.expiryTimeMillis);

			const deferUrl = 'https://www.googleapis.com/androidpublisher/v2/applications/' +
				encodeURIComponent(receipt.data.packageName) +
				'/purchases/subscriptions/' +
				encodeURIComponent(receipt.data.productId) +
				'/tokens/' + encodeURIComponent(receipt.data.purchaseToken) + ':defer' +
				'?access_token=' + encodeURIComponent(accessToken);
			const newExpiryTime = expiryTime + extendTime;
			...
			request.post({
				...
				body: {
					deferralInfo: {
				    expectedExpiryTimeMillis: expiryTime,
				    desiredExpiryTimeMillis: newExpiryTime
				  }
				}
			}, (error, res, body) => {
				if (error || !body.newExpiryTimeMillis) {
					verbose.log(NAME, 'Failed: couldnt defer payment');
					return cb(new Error('couldnt defer payment'), {
						status: constants.VALIDATION.FAILURE,
						message: 'couldnt defer payment'
					});
				}
				verbose.log('Successfully incremented billing date');
				return cb(null, body);
			});
		});
